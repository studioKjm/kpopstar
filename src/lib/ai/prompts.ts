...
export const FACT_CHECK_PROMPT: PromptTemplate = {
  ...
  systemPrompt: SYSTEM_PROMPTS.KPOP_NEWS_EXPERT,
  userPromptTemplate: `다음 기사의 사실 관계를 엄격하게 검증해주세요.
...
1. 숫자 및 날짜 일관성 검증 (최우선)
   - "X년 만에", "X개월 만에", "X일 만에" 같은 표현에서 숫자가 정확한지 확인
   - 기사 내에서 언급된 모든 날짜와 숫자가 서로 일관성 있는지 확인
   - 예: "5년 만에 컴백"이라고 했는데, 실제 마지막 활동이 4년 전이라면 오류
   - 예: "2020년 이후 5년 만"이라고 했는데, 현재 연도 기준으로 계산하면 모순이면 오류
   - 숫자 불일치는 반드시 "error" severity로 표시

2. 기사 제목과 본문 일관성
   - 제목에 나온 정보와 본문의 정보가 일치하는지 확인
   - 제목과 본문에서 서로 다른 숫자나 날짜가 나오면 오류

3. 인물/그룹명 정확도
   - 멤버 이름, 소속사, 그룹명의 정확한 표기 확인
   - 잘못된 표기나 오타 감지

4. 날짜/일정 정확도
   - 컴백일, 발매일, 공연일 등이 논리적으로 맞는지 확인
   - 과거 날짜가 미래보다 늦게 나오는 등의 오류 감지

5. 사실 관계 일관성
   - 기사 내에서 서로 모순되는 정보가 있는지 확인
   - 예: "첫 컴백"이라고 했는데 본문에 이전 컴백 언급이 있으면 모순

[중요 지침]
- 숫자나 날짜 관련 불일치는 반드시 감지하고 "error" severity로 표시
- "X년 만에", "X개월 만에", "X일 만에" 같은 표현은 정확한 날짜 계산이 필요하므로, 기사 내용만으로는 확인 불가능한 경우에도 반드시 "warning" 이상으로 표시
- 특히 "5년 만에", "4년 만에" 같은 표현은 정확한 마지막 활동 시점 확인이 필요하므로, 확인 불가능하면 "warning"으로 표시하되 반드시 언급
- 제목과 본문에서 서로 다른 숫자가 나오면 반드시 "error"로 표시
- 모든 숫자와 날짜를 꼼꼼히 검토하여 불일치를 찾아내야 함
- "X년 만에" 같은 표현이 기사 내에서 다른 정보와 모순되면 "error"로 표시
...
JSON 형식으로 응답:
{
  ...
  "issues": [
    {
      "type": "date" | "name" | "fact" | "schedule" | "number",
      "severity": "warning" | "error",
      "message": "구체적인 문제 설명 (어떤 숫자/날짜가 문제인지 명시)",
      "suggestion": "수정 제안 (선택)",
      "position": "문제가 발생한 위치 (예: '제목', '본문 첫 문단')"
    }
  ]
}`,
  outputFormat: 'json',
